...
# fix    
    def forward_features(self, x, return_all_features=False):
        ...
        # [수정됨] 모든 레이어의 Hidden State를 저장할 리스트 생성
        all_hidden_states = []
        ...
        # [수정됨] return_all_features=True면 리스트 전체 반환 (LLaVA용)
...
def load_clip_visual_state_dict(checkpoint_path: str, map_location: str = "cpu", is_openai: bool = False, skip_list: list = []):
    ...
    # [수정됨] 키 이름 변경 로직 활성화
    # 1. 'visual.'로 시작하지 않는 키(텍스트 인코더 등)는 삭제
    keys_to_remove = [k for k in state_dict.keys() if not k.startswith("visual.")]
    for k in keys_to_remove:
        del state_dict[k]
    
    # 2. 'visual.' 접두사 제거 (visual.blocks.0 -> blocks.0)
    new_state_dict = {}
...
class EVAEncoderWrapper(nn.Module):
    def __init__(self, vision_tower_pretrained, config):
        ...
        # LLaVA config에서 select_layer 값을 가져옴 (기본값 -2)
        self.select_layer = config.get("mm_vision_select_layer", -2)

    def forward(self, image, **kwargs):
        # 위에서 수정한 forward_features 덕분에 이제 리스트가 반환됨
        all_hidden_states = self.model(image, return_all_features=True)
        
        # 원하는 레이어 선택 (예: -2)
        # all_hidden_states는 [Layer0_out, Layer1_out, ..., LayerN_out] 형태
        selected_feature = all_hidden_states[self.select_layer]
        
        # CLS 토큰 제거 (모든 LLaVA 모델의 공통 전처리)
        # shape: [Batch, N_Patches + 1, Dim] -> [Batch, N_Patches, Dim]
        encode = selected_feature[:, 1:, :] 
